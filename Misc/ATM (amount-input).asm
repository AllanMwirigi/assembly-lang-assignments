...
.data
	...
	DepositAmountPrompt db "DepositAmount:$"
...
proc menuAction
	...
	finishOption:
		mov [inputCounter], 0h
		mov [inputAmount], 0h
		;reset inputAmountArr
		xor al, al
		mov di, offset inputAmountArr
		mov cx, 5
		cld
		rep stosb
...
proc readAmounts
	...
	lNumFromArr:
		...
		l4Mult: 
			mov dx, 1000
			mul dx
			add inputAmount, ax		;result of multiplication is stored in AX
			jmp lAmntTerm
		l3Mult:
			mov dx, 100
			mul dx			
...
; proc checkPIN
	; compareElem:
			; mov bx, passwdCounter		;for some reason indexing can only be done with a 16-bit register
			; mov al, pin1[bx]
			; cmp al, inputPin[bx]
			; je next
			; jne lNotEqual
